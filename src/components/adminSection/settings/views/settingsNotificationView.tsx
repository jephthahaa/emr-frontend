...
const SettingsNotificationView = () => {
  const { toggleNotifications } = useZomujoApi(false).admin.settings;
  ...
  const { mutate, isPending } = useMutation({
    ...
    mutationFn: async (data: INotificationDetails) => {
      return toggleNotifications(data);
    },
    onSuccess: () => {
        ...
        <div className="flex w-[668px] flex-row gap-20">
          ...
          <div className="flex w-[325px] shrink-0 flex-col gap-8">
            <div className="flex flex-row items-center gap-2">
              <ToggleButton
                toggled={watch("status")}
                setToggled={(value) => setValue("status", value)}
                size={24}
                variant="secondary"
              />
              <p>{watch("status") ? "On" : "Off"}</p>
            </div>
